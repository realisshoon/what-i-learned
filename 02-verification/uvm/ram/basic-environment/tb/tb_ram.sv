    ...
    virtual task run_phase(uvm_phase phase);
        ...
        forever begin
            ...
            if (active_read) begin
                ...
                `uvm_info("MON", $sformatf("Captured READ: addr=0x%0h, rdata=0x%0h",
                                           clone_item.addr, clone_item.rdata), UVM_LOW)
            ...
            if (ram_if.valid) begin
                if (ram_if.we) begin
                    ...
                    `uvm_info("MON", $sformatf("Captured WRITE: addr=0x%0h, wdata=0x%0h",
                                               clone_item.addr, clone_item.wdata), UVM_LOW)
                    send.write(clone_item);
                end else begin
...
class ram_scb extends uvm_scoreboard;
    ...
    virtual function void write(ram_seq_item data);
        ...
        if (data.we) begin
            ...
            `uvm_info("SCB", $sformatf("Write Ref Model: addr=0x%0h -> data=0x%0h", data.addr,
                                       data.wdata), UVM_LOW)
        end else begin
            ...
            if (data.rdata == expected_data) begin
                `uvm_info("SCB",
                          $sformatf(
                              "PASS! Read mismatch CHECK: addr=0x%0h, expected=0x%0h, actual=0x%0h",
                              data.addr, expected_data, data.rdata), UVM_LOW)
            end else begin
                `uvm_error("SCB", $sformatf(
                           "FAIL! Read mismatch CHECK: addr=0x%0h, expected=0x%0h, actual=0x%0h",
                           data.addr,
                           expected_data,
                           data.rdata
                           ))
