...
module multi_datapath (
  ...
  // ALU Input Mux
  mux2to1 U_MUX_ALU (
      .in0    (o_dec_rs2),    // Use registered rs2
      .in1    (o_dec_imm_e),  // Use registered imm
      .sel    (alu_src_sel),
      .out_mux(alu_rs2_mux)
  );
  ...
  // ALU (Restored missing instantiation)
  alu U_ALU (
  ...
  register_s U_EXE_RS2 (
      ...
      .rst(rst),
      .in (o_dec_rs2),
      .out(o_exe_rs2)   // Corrected output from o_dec_rs2 to o_exe_rs2
  );
...
module register_s (
  ...
  logic [31:0] register_val;
  assign out = register_val;

  always_ff @(posedge clk or posedge rst) begin
    if (rst) register_val <= 0;
    else register_val <= in;
  end
endmodule
...
module program_counter (
    ...
    input  logic        b_taken,
    input  logic        jal,
    input  logic        pc_en,       // Fixed missing comma
    input  logic        jalr,
  ...
  // PC next latch register
  register_s U_EXE_PC_NEXT (
      .clk(clk),
      .rst(rst),
      .in (pc_next),
      .out(o_exe_pc_next)  // Corrected output from o_dec_rs2 to o_exe_pc_next
  );
  ...
  always_ff @(posedge clk or posedge rst) begin
    if (rst) begin
      ...
      pc_reg <= o_exe_pc_next;  // Connected registered PC next
    end
  end
...
module imm_extend (
  ...
  always_comb begin
    ...
    case (instr_code[6:0])
      ...
      `B_TYPE:
      imm_extend = {{20{instr_code[31]}}, instr_code[7], instr_code[30:25], instr_code[11:8], 1'b0};
